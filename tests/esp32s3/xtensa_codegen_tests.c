...
static void test_helper_stub_layout(void)
{
  ...
  xtensa_emit_native_block_prologue(&ptr);
...
static void test_native_arm_add_register(void)
{
  ...
  uint8_t *ptr = code;
  bool emitted;
  static const uint8_t callx8[] = {0xE0, 0x02, 0x00};
  static const uint8_t expected[] =
  {
    0x21, 0xC1, 0xFF,
    0x42, 0x22, 0x04,
...
static void test_native_arm_immediate_literal(void)
{
  ...
  expect_u32("native MOV pc literal", load_u32_le(literal_base + 20),
             0x08000204);
  expect_size("native MOV no cycle update length", (size_t)(ptr - code), 15);
}
