...
END {
  for (wi = 1; wi <= 5; wi++) {
    for (pi = 1; pi <= 2; pi++) {
      ...
      if (name != "mapped_alu" &&
          ((insns["optimized:" key] + 0) > (insns["baseline:" key] + 0) ||
           (bytes["optimized:" key] + 0) > (bytes["baseline:" key] + 0)))
  ...
  print "result=PASS command=rv32im-perf-ab workload=all profiles=2 " \
    "frozen_workloads=5 correctness_hashes_equal=1 guard_workloads_no_regression=1 " \
