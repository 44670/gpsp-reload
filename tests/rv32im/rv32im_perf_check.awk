...
BEGIN {
  ...
  cold_instret_max["mapped_alu"] = mapped_alu_cold_instret_max
...
/ command=rv32im-perf / {
  ...
  if (workload == "mapped_alu" && phase == "warm") {
    mapped_alu_optimized_instret = field("rv32_instret") + 0
...
END {
  ...
  if (mapped_alu_optimized_instret * 100 > \
      mapped_alu_baseline_instret * (100 - mapped_alu_min_improvement))
    fail("mapped ALU warm retired-instruction improvement below target")
  ...
  improvement_x100 = int( \
    (mapped_alu_baseline_instret - mapped_alu_optimized_instret) * 10000 / \
    mapped_alu_baseline_instret)
  ...
  print "RV32IM mapped_alu optimized warm_instret=" \
    mapped_alu_optimized_instret " generated_bytes=" \
    mapped_alu_optimized_bytes " instret_reduction_percent_x100=" \
