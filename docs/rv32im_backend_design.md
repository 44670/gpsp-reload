...
- qemu-user harness `compare` execution of generated RV32IM
  ...
  and shifted-store remaining-cycle handoff,
  ...
  one hundred sixty eight runtime blocks executed, basic data-processing remaining-cycle and invalid re-lookup fallback handoffs,
  ...
  register-offset byte-store handoffs, byte-store SMC/IRQ
