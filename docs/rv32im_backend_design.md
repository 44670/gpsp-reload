...
- qemu-user harness `compare` execution of generated RV32IM
  ...
  ninety-five runtime blocks executed, ADDS/SUBS/RSBS/CMP/logical/test-op CPSR flag results and
  ...
  helper memory, helper load and load-to-PC remaining-cycle handoffs, PC-relative load/store memory,
  ...
  unsupported-block, Thumb lookup-miss, helper load, load-to-PC, direct-branch, BL, BX ARM, BX Thumb, SWI, and PC-write ADD
