...
register forms now emit directly from mapped sources to the mapped destination,
including one-instruction `MUL`; overlap, all mapped-register classes, helper
and scheduler boundaries, conditional frontend paths, and unsupported-block
discard remain covered by the exact, runtime, mixed, and external-ROM gates.
