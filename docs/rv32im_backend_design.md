...
- qemu-user harness `compare` execution of generated RV32IM
  ...
  twenty-three runtime blocks executed, ADDS/CMP CPSR flag results and
  low-bit preservation checked, MRS CPSR/SPSR read results and native PSR
  accounting checked, multiply/accumulate results checked, helper memory and
  alert observations hashed,
