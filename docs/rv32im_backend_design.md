...
- qemu-user harness `compare` execution of generated RV32IM
  ...
  ninety-four runtime blocks executed, ADDS/SUBS/RSBS/CMP/logical/test-op CPSR flag results and
  ...
  helper memory, helper load remaining-cycle handoff, load-to-PC memory, PC-relative load/store memory,
  ...
  unsupported-block, Thumb lookup-miss, helper load, direct-branch, BL, BX ARM, BX Thumb, SWI, and PC-write ADD
