...
- qemu-user harness `compare` execution of generated RV32IM
  ...
  one hundred four runtime blocks executed, ADDS/SUBS/RSBS/CMP/logical/test-op CPSR flag results and
  ...
  unsupported-block, Thumb lookup-miss, helper load, load-to-PC, PC-relative load, word store, register-offset load/store, immediate/PC-relative/register-offset halfword load, register-offset halfword store, direct-branch, BL, BX ARM, BX Thumb, SWI, and PC-write ADD
