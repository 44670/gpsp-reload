...
- qemu-user harness `compare` execution of generated RV32IM
  ...
  register-offset/shifted-LSL/shifted-LSR/shifted-ASR/RRX load ops,
  ...
  one hundred seventy one runtime blocks executed, basic data-processing remaining-cycle and invalid re-lookup fallback handoffs,
  ...
  and shifted-ASR load, subtract-offset, RRX load/store, and register-offset writeback
...
The lower-level standalone runtime test also emits LSL/LSR/ASR/ROR/RRX
register-offset load blocks and LSL/LSR/ASR/ROR/RRX register-offset store blocks, then
checks their helper address, PC, value, and leftover-cycle handoff observations
directly.
