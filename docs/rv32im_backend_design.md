...
- qemu-user harness `compare` execution of generated RV32IM
  ...
  one hundred one runtime blocks executed, ADDS/SUBS/RSBS/CMP/logical/test-op CPSR flag results and
  ...
  source-PC store, register-offset byte-store remaining-cycle handoff, byte-store SMC/IRQ
  ...
  immediate, PC-relative, and register-offset halfword remaining-cycle handoffs, writeback halfword
  ...
  unsupported-block, Thumb lookup-miss, helper load, load-to-PC, PC-relative load, register-offset load/store, immediate/PC-relative/register-offset halfword load, direct-branch, BL, BX ARM, BX Thumb, SWI, and PC-write ADD
